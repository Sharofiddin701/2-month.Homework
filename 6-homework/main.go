...
func main() {
	...
	fmt.Println("5-Mamlakatni qidirish:")
	...
	case 5:
		...
		fmt.Println("1-Mamlakatni qidirish")
		fmt.Println("2-Datada bor barcha mamlakatlarni chiqarish")
		_, err := fmt.Scan(&searchQuery)
		if err != nil {
			fmt.Println("Foydalanuvchi kiritishda xato:", err)
			return
		}
		switch searchQuery {
		case "1":
			var searchTerm string
			fmt.Println("Qidirish so'zini kiriting:")
			_, err = fmt.Scan(&searchTerm)
			if err != nil {
				fmt.Println("Qidirish so'zini o'qishda xato:", err)
				return
			}

			foundCountries, err := inv.SearchByName(searchTerm)
			if err != nil {
				fmt.Println("Mamlakatni qidirishda xato:", err)
				return
			}
			fmt.Println("Qidirilgan mamlakat: ", foundCountries)
		case "2":

			allCountries, err := inv.GetAll()
			if err != nil {
				fmt.Println("Mamlakatlar olishda xato:", err)
				return
			}
			fmt.Println("Mamlakat: ", allCountries)
		default:
