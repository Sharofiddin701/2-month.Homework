...
func (inv *Inventory) SearchByName(name string) ([]country.Country, error) {
    rows, err := inv.db.Query("SELECT id, name, code FROM countries WHERE name LIKE '%' || $1 || '%'", name)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
    ...
    for rows.Next() {
        ...
        foundCountries = append(foundCountries, c)
    }
